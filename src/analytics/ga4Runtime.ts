...
const DEFAULT_CONTEXT: BookAnalyticsContext = {
  book_id: 'playwrite-clik-book-with-ide',
  version: 'v2',
};
...
export function markOnce(key: string): boolean {
  ...
  try {
    ...
    const fallbackWindow = window as unknown as Record<string, unknown>;
    if (fallbackWindow[fallbackKey]) {
      return false;
    }
    fallbackWindow[fallbackKey] = true;
    return true;
  }
}
